...
public class UpdateSqlConvert {
   public static String process(SQLUpdateStatement updateStatement) {
      if (updateStatement.getTableSource() instanceof SQLJoinTableSource) {
         return "";
      } else {
         ...
         String beginLines = String.format("update %s set ", tableName);
         StringBuilder sql = new StringBuilder(beginLines);
         Iterator var5 = items.iterator();

         while(var5.hasNext()) {
            SQLUpdateSetItem item = (SQLUpdateSetItem)var5.next();
            if (item.getValue() instanceof SQLMethodInvokeExpr) {
               return "";
            }
            ...
            if (value != null && valueString.startsWith("\"") && valueString.endsWith("\"")) {
               valueString = "'" + valueString.substring(1, valueString.length() - 1) + "'";
            }

            String currrentColAndVal = String.format("%s = %s,", columnName, valueString);
            sql.append(currrentColAndVal);
         }
